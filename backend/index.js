...
app.post('/api/empleados', async (req, res) => {
  ...
  if (!nombre || !puesto || !horario) return res.status(400).json({ error: 'Faltan datos' });
  const empleados = db.collection('empleados');
  const nuevo = { id: String(Date.now()), nombre, puesto, horario, rol: 'empleado', estado: 'inactivo', vinculado: false };
...
app.post('/api/vincular', async (req, res) => {
  ...
  if (!idEmpleado || !nombreDispositivo) return res.status(400).json({ error: 'Faltan datos' });
  const empleados = db.collection('empleados');
  ...
  await dispositivos.insertOne({ idEmpleado, nombreDispositivo, activo: true, timestamp: new Date().toISOString() });
  res.json({ success: true, mensaje: 'Dispositivo vinculado' });
});
